...
public class Ontology {
    ...
    {   Vertices result = null;
        if (v.getValue().equals(value)) result =  v;
        else
    ...
    public Vertices findCommonParent(String featureVal, String dataV)
    {
        ...
        if (this.search(hypNode, dataV) != null) return  hypNode;
        if (this.search(dataNode, featureVal) != null) return dataNode;
    ...
    {
        if (v.getChild().isEmpty()) v.addChild("-");
        else
        {
            for (Vertices child: v.getChild()) addStopper(child);
        }
    }
}
