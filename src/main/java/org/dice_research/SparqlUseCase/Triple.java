...
public class Triple {
	...
	/**
	 * Returns BitSet of booleans where the indices which are set to true indicate
	 * where the two triples being compared dissagre
	 * 
	 * @param t Triple to be compared against
	 * @return a BitSet whose set bits define the position/s (0 for subject, 1 for
	 *         predicate, 2 for object) at which this triple disagrees with triple t
	 */
	...
	/**
	 * Compares this triple against triple t, returns true only if all their triple
	 * values are equal
	 * 
	...
	public boolean hasSameValues(Triple t) {
		if (this.s.toString().equals(t.s.toString()) && this.p.toString().equals(t.p.toString())
				&& this.o.toString().equals(t.o.toString())) {
			return true;
		}
		return false;
	}
	 ...
	 * (similar in terms of the lowest dissagrement)
	 * 
